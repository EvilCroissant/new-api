...
func TestMarkFailedChannelForRetryFallsBackWhenNoAlternativeExists(t *testing.T) {
	const (
		groupName = "controller-retry-channel-fallback-test"
		modelName = "controller-retry-channel-fallback-model"
		channelID = 996003
	)
	priority := int64(10)
	weight := uint(100)
	...
	require.NoError(t, model.DB.Create(&model.Channel{
		Id:       channelID,
		Type:     constant.ChannelTypeOpenAI,
		Name:     "retry-channel-fallback-test",
		Key:      "test",
		Status:   common.ChannelStatusEnabled,
		Models:   modelName,
		Group:    groupName,
	...
	failed, err := model.GetChannelById(channelID, true)
	require.NoError(t, err)
	markFailedChannelForRetry(c, retryParam, failed, false)
