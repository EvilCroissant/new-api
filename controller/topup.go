...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		settledTopUp, quotaToAdd, applied, err := model.RechargeEpay(verifyInfo.ServiceTradeNo, verifyInfo.Type)
		if err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 结算充值失败 trade_no=%s user_id=%d client_ip=%s error=%q topup=%q", topUp.TradeNo, topUp.UserId, c.ClientIP(), err.Error(), common.GetJsonString(topUp)))
			return
		}
		if applied {
			logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 充值成功 trade_no=%s user_id=%d client_ip=%s quota_to_add=%d money=%.2f topup=%q", settledTopUp.TradeNo, settledTopUp.UserId, c.ClientIP(), quotaToAdd, settledTopUp.Money, common.GetJsonString(settledTopUp)))
			model.RecordTopupLog(settledTopUp.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%f", logger.LogQuota(quotaToAdd), settledTopUp.Money), c.ClientIP(), settledTopUp.PaymentMethod, "epay")
		}
	} else {
