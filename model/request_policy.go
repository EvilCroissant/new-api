...
func BuildRequestPolicy(options map[string]string) (*RequestPolicySnapshot, error) {
	...
	for key, value := range raw {
		if field, ok := strings.CutPrefix(key, "channel_affinity_setting."); ok {
			switch field {
			case "enabled", "optimization_enabled", "frt_optimization_enabled", "session_mode", "frt_consecutive_slow_limit", "frt_all_slow_hold_seconds", "switch_on_success", "keep_on_channel_disabled", "max_entries", "default_ttl_seconds", "upward_probe_interval_seconds", "rules":
			default:
	...
	if snapshot.Affinity.FRTProbeCount < 1 || snapshot.Affinity.FRTProbeCount > 10 {
		return nil, fmt.Errorf("invalid FRT consecutive slow limit")
	}
	if snapshot.Affinity.FRTProbeCooldownSeconds < 1 || snapshot.Affinity.FRTProbeCooldownSeconds > 3600 {
		return nil, fmt.Errorf("invalid FRT all-slow hold")
	}
	if snapshot.Affinity.UpwardProbeIntervalSeconds < 1 {
		return nil, fmt.Errorf("invalid channel optimization interval")
	}
