	...
	for _, dialect := range []string{"sqlite", "mysql", "postgres"} {
		t.Run(dialect, func(t *testing.T) {
			...
			require.NoError(t, UpdateRequestPolicyOptions(map[string]string{
				...
				"channel_affinity_setting.upward_probe_interval_seconds": "1800",
			}))
			assert.True(t, CurrentRequestPolicy().Affinity.FRTOptimizationEnabled)
			assert.Equal(t, 3, CurrentRequestPolicy().Affinity.FRTProbeCount)
			assert.Equal(t, 120, CurrentRequestPolicy().Affinity.FRTProbeCooldownSeconds)
			assert.Equal(t, 1800, CurrentRequestPolicy().Affinity.UpwardProbeIntervalSeconds)
