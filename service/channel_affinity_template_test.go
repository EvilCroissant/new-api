		...
		for _, tc := range []struct {
			...
			t.Run(globalMode+"/"+tc.name, func(t *testing.T) {
				...
				*setting = snapshot.Affinity
				...
				cache := getChannelAffinityCache()
				initialState := buildChannelAffinityStateForTest(1, time.Minute)
				require.NoError(t, cache.SetWithTTL(cacheKey, initialState, time.Minute))
				...
				if expected == "off" {
					...
					require.NoError(t, err)
					assert.True(t, found)
					assert.Equal(t, initialState, cached, "off does not refresh session bindings")
