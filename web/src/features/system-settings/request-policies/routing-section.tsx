...
function AffinitySettings() {
  ...
  const optimizationEnabled = useWatch({
    control: form.control,
    name: 'channel_affinity_setting.optimization_enabled',
  })
  const frtOptimizationEnabled = useWatch({
    control: form.control,
  ...
  return (
    ...
    >
      ...
      <Accordion className='mt-4'>
        ...
        <SettingsAccordion
          value='channel-optimization'
          title={t('Channel Optimization')}
          keepMounted
        >
          <div className='space-y-4'>
            <FormField
              ...
              render={({ field }) => (
                <SettingsSwitchField
                  controlId='channel_affinity_setting.optimization_enabled'
                  disabled={!enabled}
                  checked={field.value}
                  onCheckedChange={field.onChange}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>
                    {t('Channel optimization interval (seconds)')}
                  </FormLabel>
                  <FormControl>
                    <Input
                      type='number'
                      min={1}
                      step={1}
                      disabled={!enabled || !optimizationEnabled}
                      {...safeNumberFieldProps(field)}
                    />
                  </FormControl>
                  <FormMessage />
            ...
            <FormField
              ...
              render={({ field }) => (
                <SettingsSwitchField
                  controlId='channel_affinity_setting.frt_optimization_enabled'
                  disabled={!enabled}
                  checked={field.value}
                  onCheckedChange={field.onChange}
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          ...
                          max={item.max}
                          step={1}
                          disabled={!enabled || !frtOptimizationEnabled}
                          {...safeNumberFieldProps(field)}
                        />
                      </FormControl>
                      <FormMessage />
